...
connectToDatabase().then((connectedClient) => {
  ...
  transporter.verify().then(() => {
    ...
    app.post('/forgot-password', async (req, res) => {
      ...
      try {
          const usersCollection = connectedClient.db('personalfitness').collection("users");
          const user = await usersCollection.findOne({ email });
          if (!user) {
              return res.status(404).json({ message: 'User not found' });
          }
          const otp = generate({ length: 6, charset: 'numeric' });
          const passwordResetSessions = connectedClient.db('personalfitness').collection("password-reset-sessions");
    ...
    app.post('/chat', async (req, res) => {
        ...
        try {
            const chatsCollection = connectedClient.db('personalfitness').collection("chats");
